...
{
    [JsonConstructor]
    public PagedList(int pageNumber, int pageSize, IEnumerable<T> items, int totalItemCount) : this(items,
        new Pagination { PageNumber = pageNumber, PageSize = pageSize }, totalItemCount)
    {
    ...
    public PagedList(IQueryable<T> queryable, IPagination page) : this(page, queryable?.Count() ?? 0)
    {
        if (TotalItemCount > 0)
        {
            this.Items =
                (page.PageNumber == 1
                    ? queryable.Take(page.PageSize)
                    : queryable.Skip((page.PageNumber - 1) * page.PageSize).Take(page.PageSize)).ToArray();
        }
    }
